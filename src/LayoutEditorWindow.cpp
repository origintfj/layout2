...
// OpenGL backend:
// - detailed/simplified/coarse modes all submit geometry via GL,
// - detailed mode applies layer stipple in fragment shader for parity.
...
private:
    ...
    static quint64 hashRenderItems(const QVector<RenderItem>& items, const QSize& viewportSize) {
    ...
    void rebuildCachedGeometry(const QVector<RenderItem>& items) {
        ...
        for (auto it = styleBuckets.cbegin(); it != styleBuckets.cend(); ++it) {
            ...
            for (const RenderItem* itemPtr : it.value()) {
                ...
                for (int i = 1; i < vertexCount - 1; ++i) {
                    ...
                    appendVertex(m_cachedTriangleVertexData,
                                 origin.x(), origin.y(), r, g, b, a);
                    appendVertex(m_cachedTriangleVertexData,
                                 p1.x(), p1.y(), r, g, b, a);
                    appendVertex(m_cachedTriangleVertexData,
                                 p2.x(), p2.y(), r, g, b, a);
    ...
    void appendPolygonTriangles(QVector<float>& out,
                               const QPolygonF& polygon,
                               const QColor& color) {
        const int vertexCount = polygon.size();
        if (vertexCount < 3) {
    ...
    void drawDetailedItemsWithGl(QOpenGLFunctions* gl, const int stride) {
        ...
        for (const RenderItem& item : m_cachedDetailedItems) {
            ...
            const std::array<float, 8> rows = patternRowsFor(item.pattern);
    ...
    QVector<RenderItem> m_cachedDetailedItems;
...
private:
    ...
    QVector<SelectedObjectDescriptor> selectedObjectDescriptors() const {
    ...
    static QWidget* makeEmptyPropertiesPane(QWidget* parent) {
    ...
    static QWidget* makeGroupPropertiesPane(const QString& typeName,
    ...
    static QWidget* makeRectanglePropertiesPane(const SelectedObjectDescriptor& descriptor, QWidget* parent) {
        auto* panel = new QWidget(parent);
        auto* form = new QFormLayout(panel);
    ...
    static QWidget* makeGenericPropertiesPane(const SelectedObjectDescriptor& descriptor, QWidget* parent) {
        auto* panel = new QWidget(parent);
        auto* form = new QFormLayout(panel);
        form->addRow("Type:", new QLabel(descriptor.typeName, panel));
        form->addRow("Object ID:", new QLabel(QString::number(descriptor.objectId), panel));
    ...
    void showPropertiesDialog() {
        ...
        const QVector<SelectedObjectDescriptor> descriptors = selectedObjectDescriptors();
...
bool LayoutEditorWindow::eventFilter(QObject* watched, QEvent* event) {
    ...
    if (watched == m_layerTable && event->type() == QEvent::KeyPress) {
        ...
        if (keyEvent->key() == Qt::Key_Q && keyEvent->modifiers() == Qt::NoModifier) {
